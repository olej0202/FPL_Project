...
def Generate_Player_Historical():
    ...
    Cols_to_include=["name", "position", "kickoff_time", "opponent_team", "season", "assists", "bonus", "expected_assists", "expected_goals", "goals_scored", "minutes", "total_points", "ICT", "Adjusted_XG", "Adjusted_XA"]
    filtered_data=filtered_data[Cols_to_include]
    merged_df = filtered_data.merge(team_code, how='left', left_on='opponent_team', right_on='code')

    merged_df.drop(columns=['opponent_team','code' ], inplace=True)
