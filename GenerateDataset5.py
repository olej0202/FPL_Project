...
def main_Transform():
    ...
    for index, row in unique_players.iterrows():
        ...
        if (len(player_df) > (4)) and (player_df["minutes"].sum() > 100):
            ...
            player_df['defcon_avg_hit_rate_T1'] = player_df['defcon_hit_rate_T1'].where(player_df['defcon'] > 0).rolling(30, min_periods=1).mean()*0.7+0.3*player_df['defcon_hit_rate_T1'].where(player_df['defcon'] > 0).rolling(10, min_periods=1).mean()
            player_df['defcon_avg_hit_rate_T2'] = player_df['defcon_hit_rate_T2'].where(player_df['defcon'] > 0).rolling(30, min_periods=1).mean()*0.7+0.3*player_df['defcon_hit_rate_T2'].where(player_df['defcon'] > 0).rolling(10, min_periods=1).mean()
            player_df['defcon_avg_hit_rate_T0'] = player_df['defcon_hit_rate_T0'].where(player_df['defcon'] > 0).rolling(30, min_periods=1).mean()*0.7+0.3*player_df['defcon_hit_rate_T0'].where(player_df['defcon'] > 0).rolling(10, min_periods=1).mean()
            player_df['defcon_avg_hit_rate_T3'] = player_df['defcon_hit_rate_T3'].where(player_df['defcon'] > 0).rolling(30, min_periods=1).mean()*0.7+0.3*player_df['defcon_hit_rate_T3'].where(player_df['defcon'] > 0).rolling(10, min_periods=1).mean()
