    ...
    for i in range(len(players)):
        ...
        for h in range(len(df)):
            ...
            if(pred_variable=="GOALS"):
               ...
               stat_pred=df['Goal_Statistics_share'].values[h]*0.3+df['Rolling_adjusted_XG'].values[h]*0.2+df['Big_Chances'].values[h]*0.2*0.33+df['Share_of_XG'].values[h]*0.2+df['Share_of_XG_Short'].values[h]*0.1
            ...
            if(pred_variable=="Assist"):
               ...
               stat_pred=df['Assist_Statistics_share'].values[h]*0.3+df['Rolling_adjusted_XA'].values[h]*0.2+df["Big_Chances_Created"].values[h]/2 * 0.2+df['Share_of_XA'].values[h]*0.2+df['Share_of_XA_Short'].values[h]*0.1
               player_model.append(pred)
    ...
    for j in range(len(unique_players)):
        ...
        if(position=="FWD"):
            summary_dataset["Points_prediction"]=(2+summary_dataset["Goal_pred"]*5.2
                                                  ...
                                                  -summary_dataset["Card_pred"] )*0.9+0.1*summary_dataset["Fantasy_pred"]+summary_dataset["CBI_pred"]*0.6*2
            ...
            summary_dataset["Points_prediction"]=(2
                                                  +summary_dataset["Bonus_pred"]
                                                  + (30 - np.minimum(30, summary_dataset["GC_pred"]*100)) / -28
