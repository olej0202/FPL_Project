    ...
    for i in range(len(players)):
        ...
        for h in range(len(df)):
            ...
            if(pred_variable=="bps"):
               real_variable="bonus" 
               player_preds.append(max(df['Rolling_adjusted_BPS'].values[h]*0.4+df['Rolling_adjusted_BPS_2'].values[h]*0.6,5)*np.minimum(1, df["average_minutes"].values[h] / 80))
...
def Generate_point_predictions(GW_list):
    ...
    for j in range(len(unique_players)):
        ...
        if(position=="FWD"):
            summary_dataset["Bonus_pred"]=summary_dataset["Bonus_pred2"]*0.5+0.5*expected_bonus(
