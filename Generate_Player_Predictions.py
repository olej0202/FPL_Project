    ...
    for i in range(len(players)):
        ...
        for h in range(len(df)):
            ...
            if(pred_variable=="Assist"):
               ...
               team_data_xa_pred=(df['Understat_POSXA'].values[h]*df["opposition_xgc"].values[h]+df['Understat_POSXA_Share'].values[h]*team_xg*1.1)*(0.5)
    ...
    for j in range(len(unique_players)):
        ...
        for i in range(len(player_data)):
            ...
            try:
                ...
                bps.append(max(0.5,(xgb_bps_player["pred"].values[i]*0.3
                                   ...
                                   +max(0.5,cluster_bps_player["pred"].values[i]*0.1*0.04)))*0.7)
            except:
                bps.append(0)
        ...
        if(position=="FWD"):
            ...
            summary_dataset["Points_prediction"]=(1+summary_dataset["Goal_pred"]*6
                                                  ...
                                                  +summary_dataset["Bonus_pred"]
                                                  +summary_dataset["GC_pred"]*5)*0.8+0.2*summary_dataset["Fantasy_pred"]+((summary_dataset["CBI_pred"]/10)*0.7+0.3*cbi_hit_rate)*0.85*2
