    ...
    for i in range(len(players)):
        ...
        for h in range(len(df)):
            ...
            if(pred_variable=="GOALS"):
               ...
               stat_pred=df['Rolling_adjusted_XG_share'].values[h]*0.27+df['Rolling_adjusted_XG_per90_share'].values[h]*0.27+df['Rolling_adjusted_Threat_per90_share'].values[h]*0.27+df['Big_Chances'].values[h]*0.2*0.33
               ...
               team_data_xg_pred=(df['Understat_POSXG'].values[h]*df["opposition_xgc"].values[h]*0+1*df['Understat_POSXG_Share'].values[h]*team_xg)
            ...
            if(pred_variable=="Assist"):
               ...
               stat_pred=df['Rolling_adjusted_XA_share'].values[h]*0.2+df['Rolling_adjusted_XA_per90_share'].values[h]*0.2+df['Rolling_adjusted_creativity_per90_share'].values[h]*0.2+df['rolling_Assist_historic'].values[h]*0.2+df["Big_Chances_Created"].values[h]/2 * 0.2
               pred=pred*0.7+0.3*stat_pred
               ...
               team_data_xa_pred=(df['Understat_POSXA'].values[h]*df["opposition_xgc"].values[h]*0+1*df['Understat_POSXA_Share'].values[h]*team_xg)
            ...
            if(pred_variable=="bps"):
               real_variable="bonus" 
               player_preds.append(max(df['Rolling_adjusted_BPS'].values[h]*0.4+df['Rolling_adjusted_BPS_2'].values[h]*0.6,5)*0.03*other_metric)
...
def Generate_point_predictions(GW_list):
    ...
    for j in range(len(unique_players)):
        ...
        for i in range(len(player_data)):
            try:
                goals.append((((xgb_goals_player["75"].values[i]*0.5+0.5*xgb_goals_player["25"].values[i])*0.15
