...
export default function Team_Predictions() {
  ...
  const winTextClass = (p) => {
    ...
    return prob >= 0.4 ? "text-emerald-700 font-semibold" : "text-slate-700";
  };
  ...
  const csBadgeClass = (p) => {
    const prob = normalizeProb(p);
    if (!Number.isFinite(prob)) return "bg-slate-100 text-slate-600 border-slate-300";
    if (prob >= 0.35) return "bg-emerald-100 text-emerald-800 border-emerald-300";
    if (prob <= 0.25) return "bg-red-100 text-red-700 border-red-300";
  ...
  const movementIcon = (m) => {
    ...
    if (mv === "up") return <ArrowUp size={16} className="text-emerald-700" />;
    if (mv === "down") return <ArrowDown size={16} className="text-red-600" />;
    return <Dot size={18} className="text-slate-500" />;
  };
  ...
  const tableShowLoading = tableLoading || (showPredTable && tableIsEmpty);
    ...
    if (rank === 2) return "bg-emerald-50 border-emerald-200";
    if (rank === 3) return "bg-teal-50 border-teal-200";
    if (rank === 4) return "bg-cyan-50 border-cyan-200";

    // 5th orange
    if (rank === 5) return "bg-amber-50 border-amber-200";

    // bottom 3 red
    if (rank >= Math.max(1, totalTeams - 2)) return "bg-red-50 border-red-200";
      ...
      <div className="mx-auto max-w-6xl px-3 sm:px-4 py-6 sm:py-10">
        ...
        <header className="mb-6 sm:mb-8 flex flex-col items-center gap-3 text-center">
          ...
          {uniqueGWs.length > 0 && selectedGW != null && (
            <div className="w-full max-w-md h-1.5 bg-slate-200 rounded overflow-hidden">
              <div
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center mb-6 gap-3 sm:gap-4">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-xl px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 shadow-sm text-sm sm:text-base transition focus:outline-none focus:ring-0 active:scale-[0.99]"
            ...
            {filteredData.map((match, idx) => {
              ...
              return (
                <div
                  ...
                  className={`group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm backdrop-blur-sm transition-all duration-300 ease-out will-change-transform ${
                    mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"
                  }`}
                >
                  <div className="hidden sm:grid grid-cols-[minmax(0,1fr)_64px_96px] items-center gap-3 mb-2 text-[11px]">
                    <span className="uppercase tracking-wide text-slate-500">Match</span>
                    <span className="uppercase tracking-wide text-slate-500 text-right">
                      Score
                    </span>
                    <span className="uppercase tracking-wide text-slate-500 text-right">
                      CS odds
                  ...
                  <div className="mt-1 mb-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-500">Draw</span>
      ...
      {showPredTable && (
        ...
        >
          ...
          <div className="relative mx-auto w-full max-w-xl px-3 sm:px-6 pt-20 pb-10">
            <div className="rounded-2xl border border-slate-200 bg-white/95 shadow-xl overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-xl p-1.5 border border-slate-200 bg-white hover:bg-slate-50 transition"
              ...
              <div className="max-h-[70vh] overflow-y-auto overscroll-contain">
                {tableShowLoading ? (
                  <div className="p-4 text-center text-slate-500">Loading predicted table…</div>
                  ...
                  <div className="p-2 sm:p-3">
                    <div className="grid grid-cols-[40px_26px_minmax(0,1fr)_88px_88px] text-[10px] uppercase tracking-wide text-slate-500 px-2 pb-1">
                      <span>#</span>
                      <span />
                      <span>Team</span>
                            ...
                            <div className="grid grid-cols-[0px_26px_minmax(0,1fr)_88px_88px] items-center gap-2">
                              ...
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <div className="text-sm font-medium text-slate-800 leading-tight whitespace-normal break-words">
  {t.name}
</div>
              ...
              <div className="px-3 py-2 border-t border-slate-200 text-xs text-slate-500">
                Tip: Press <span className="text-slate-600">Esc</span> to close.
              </div>
            </div>
          </div>
        </div>
